...
import fastparse._
import zio.{ IO, ZIO }
import zmail.smtp.Smtp.{ Domain, Ehlo, Helo, _ }
...
object SmtpParser {
  ...
  def mailfrom[_: P]: P[MailFrom] =
    P("MAIL FROM:" ~ reversepath ~ (" " ~ mailparameters).?).map(p => MailFrom(ReversePath(p)))
  def rcptto[_: P]: P[RcptTo]     = P("RCPT TO:" ~ forwardpath ~ (" " ~ mailparameters).?).map(p => RcptTo(ReversePath(p)))
  def data[_: P]: P[Data.type]    = P("DATA").map(_ => Data)
  def quit[_: P]: P[Quit.type]    = P("QUIT").map(_ => Quit)

  def command[_: P]: P[Command] = P((helo | ehlo | mailfrom | rcptto | data | quit) ~ EOL)
