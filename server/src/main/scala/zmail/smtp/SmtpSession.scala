...
import zmail.smtp.ReplyCode._
import zmail.smtp.Smtp._
...
case class SmtpSession(channel: AsynchronousSocketChannel, log: Log = new Log()) {
  ...
  private def writeToChannel(txt: String, channel: AsynchronousSocketChannel) =
    ZStream
      .fromChunk(Chunk.fromArray(txt.toCharArray))
      .transduce(Charset.Standard.utf8.newEncoder.transducer())
      .foreachChunk(chunk => channel.writeChunk(chunk))
  ...
  private def readLineFromChannel =
    ZStream
      .fromEffectOption(
        channel.readChunk(MAX_LINE_LENGTH).tap(c => putStrLn(s"Read chunk of length ${c.size}")).orElseFail(None)
      )
      .flattenChunks
